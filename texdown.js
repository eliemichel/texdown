 ...
 * 
 * Mainly based on marked, a markdown parser
...
function makeDeco (s) {
	return  '<span class="title">' + s + '[ <span class="label_env">(LABEL)</span>]</span>';
}
...
block.texdown.env = replace(block.texdown.env)
	('paragraph', block.texdown.paragraph)
	();
...
Lexer.prototype.token = function(src, top) {
  ...
  while (src) {
    ...
    // environment starting (texdown)
    if (top && (cap = this.rules.env_start.exec(src))) {
      src = src.substring(cap[0].length);
      this.tokens.push({
        type: 'env_start',
        env: cap[1],
    ...
    // environment ending (texdown)
    if (top && (cap = this.rules.env_end.exec(src))) {
      src = src.substring(cap[0].length);
      this.tokens.push({
        type: 'env_end'
      });
    ...
    // environnement (texdown)
    if (top && (cap = this.rules.env.exec(src))) {
      src = src.substring(cap[0].length);
      this.tokens.push({
        type: 'env',
        env: cap[1],
        label: cap[2],
        text: cap[3][cap[3].length-1] === '\n'
          ? cap[3].slice(0, -1)
          : cap[3]
      });
      continue;
...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (cap = this.rules.latex.exec(src)) {
      src = src.substring(cap[0].length);
      out += '<span class="tex2jax_process">$'
        + cap[1]
        + '$</span>';
      continue;
    }
...
Parser.prototype.tok = function() {
  switch (this.token.type) {
    ...
    case 'env_end': {
      return '</div>' + this.deco_stack.pop() + '\n';
    }
    case 'env': {
      var env = this.token.env,
          label = this.token.label,
